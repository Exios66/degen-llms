# Casino Offerings

A complete catalog of everything on the Mandalay Bay casino floor and resort.

![Main casino lobby — floor hub](images/casino-floor.png)

Seven floors, eleven activities, one chip wallet:

| Floor | Activities | Min bet |
|-------|------------|---------|
| **Table Games** | Blackjack, Texas Hold'em, Mandalay Roulette, Craps | 10 chips |
| **Slot Machines** | 14 machines incl. Megabucks & linked progressives | 1 chip |
| **Lottery Counter** | Pick 3/4, Mega/Powerball, premium scratchers | Varies (stake-scaled) |
| **Sports Book** | Scenario board, parlays/futures + prediction markets | 10 chips |
| **Trading Floor** | Futures & call/put options (NYSE, commodities, crypto) | 25 chips |
| **Racing Pavilion** | Mandalay Racing (thoroughbred sim) | 5 chips |
| **Equestrian Arena** | Dressage competition, show jumping | 10 chips |

## Stake tiers

Choose a tier before playing any machine or table. Each tier sets min/max wagers for the session:

| Tier | Range | Notes |
|------|-------|-------|
| Penny & Low Limit | $1 – $25 | Micro stakes |
| Standard | $5 – $100 | Main floor |
| High Limit | $25 – $500 | High-limit room |
| **401K Contribution** | **$542 – $6,500** | Avg. U.S. employee deferral |
| **High Roller / No Limit** | **$2,500 – bankroll** | Salon stakes — no table maximum |

## Table Games

![Table Games floor — blackjack, hold'em, roulette](images/table-games.png)

### Blackjack
Classic Vegas 21 — solo or full table with AI players. 6-deck shoe, H17, 3:2 blackjack, double/split/surrender/insurance.

→ [[Blackjack]]

### Texas Hold'em
No-limit 5-player table (you + 4 bots). Full street flow: preflop → flop → turn → river. Custom raise amounts and all-in supported. Buy-in persists across hands.

→ [[Table-Games#texas-holdem]]

### Mandalay Roulette
American wheel (0 and 00). Inside/outside bets with standard payouts.

→ [[Table-Games#roulette]]

### Craps
Pass Line / Don't Pass with come-out and point phases. Side bets: Field, Any Craps, Hard Ways, Yo-11.

→ [[Table-Games#craps]]

## Slot Machines

![Slot Machines floor — Mandalay Bay Slots](images/slot-machines.png)

Fourteen machines from penny progressives to high-limit classics:

| Machine | Bet range | Type |
|---------|-----------|------|
| Mandalay Fortune | $5 – $50 | Classic |
| High Roller | $25 – $500 | High limit |
| **Megabucks** | $1 – $3 | Wide-area progressive |
| Wheel of Fortune | $1 – $25 | Video |
| Blazin' 7s | $1 – $25 | Classic progressive |
| Buffalo Gold | $1 – $50 | Video |
| Monte Carlo | $1 – $5 | Linked progressive |
| Super Spin | $1 – $5 | Linked progressive |
| Triple Red Hot 7s | $1 – $25 | Classic |
| Double Jackpot | $1 – $25 | Video |
| Spooky Link | $1 – $25 | Themed video |
| Wizard of Oz | $1 – $25 | Themed video |
| Emerald Guardian | $1 – $25 | Themed video |
| Tiger and Dragon | $1 – $50 | Themed video |

Progressive jackpots (Megabucks, Monte Carlo, Super Spin) persist in your save. **Max bet required** to qualify.

→ [[Slot-Machines]]

## Lottery Counter

| Ticket | Description |
|--------|-------------|
| **Strip Pick 3 / Pick 4** | Digit draws — straight or box (+ high-limit SKUs) |
| **Mandalay Mega / Salon Powerball** | 5 lucky numbers (1–70) + Powerball (1–25) |
| **Scratchers** | Gold, Wild, Platinum, Diamond instant tickets |

→ [[Lottery-Counter]]

## Sports Book

![Sports Book floor — wagering and prediction markets](images/sportsbook.png)

Simulated wagering across NFL, NBA, MLB, MLS, NHL, NCAA, UFC, tennis, and golf. Bet types: moneyline, spread, totals, props, outrights.

### Prediction markets

Binary YES/NO contracts in Polymarket style:

| Category | Content |
|----------|---------|
| **Sports Pulse** | Spread/total contracts linked to board events |
| **Headlines & Buzz** | Award shows, viral stories |
| **Vegas & Resort** | Strip traffic, pool attendance |
| **History Desk** | Real historical events with fixed resolutions |
| **Easter Eggs** | Ludicrous scenarios (alien disclosure, buffet infinity, etc.) |

→ [[Sports-Book-and-Prediction-Markets]]

## Trading Floor

**Mandalay Markets** — long-only futures and call/put options on NYSE, commodities, and crypto (min 25 chips). Stored catalog with filterable pages.

→ [[Trading-Floor]]

## Racing Pavilion

Thoroughbred sim with win/place/show wagering. Real horse names, strength-based odds, animated race results.

→ [[Racing-and-Equestrian]]

## Equestrian Arena

| Event | Description |
|-------|-------------|
| **Dressage** | Scored routine competition |
| **Show Jumping** | Fault-counted round |

→ [[Racing-and-Equestrian]]

## Beyond the pits

| Amenity | Description |
|---------|-------------|
| **Cashier** | Buy chips, cash out, view ledger |
| **Off-Strip Bank Account** | Park winnings outside the cage (web) |
| **Casino Floor — shopping & bars** | Mandalay Place sky bridge, three bars, intoxication |
| **Staff Manifest** | Editable dealer roster with session overrides |
| **High Limit salon** | Chip-gated VIP tables (web) |
| **Foundation Room** | Tier-gated lounge (web + RPG) |

## Resort (off the floor)

| Zone | Highlights |
|------|------------|
| **Hotel** | Front desk, hallway mini-game, 17 room vignettes |
| **Pool complex** | Wave pool, cabanas, Shark Reef, beach club |
| **MGM Rewards** | Tier progression, comps, phone hotline |

→ [[Resort-Hotel]] · [[Pool-Complex]] · [[MGM-Rewards]]

## Pixel RPG access

Every major offering above is playable as an in-world encounter in the pixel RPG — no terminal redirect required.

→ [[Pixel-RPG-Simulator]]
