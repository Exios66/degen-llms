# Casino Offerings

A complete catalog of everything on the Mandalay Bay casino floor and resort.

Eight floors, twelve activities, one chip wallet:

| Floor | Activities | Min bet |
|-------|------------|---------|
| **Table Games** | Blackjack, Texas Hold'em, Mandalay Roulette, Craps | 10 chips |
| **Slot Machines** | 14 machines incl. Megabucks & linked progressives | 1 chip |
| **Lottery Counter** | Pick 3, Pick 4, Mega draw, scratchers | Varies |
| **Sports Book** | Scenario board, parlays/futures + prediction markets | 10 chips |
| **Trading Floor** | Futures & call/put options (NYSE, commodities, crypto) | 25 chips |
| **Arcade Alley** | CRT cabinets — Strip Cross, Neon Invaders, Breakout, Showgirl Beat | 5 chips |
| **Racing Pavilion** | Mandalay Racing (thoroughbred sim) | 5 chips |
| **Equestrian Arena** | Dressage competition, show jumping | 10 chips |

## Stake tiers

Choose a tier before playing any machine or table. Each tier sets min/max wagers for the session:

| Tier | Tables | Slots | Notes |
|------|--------|-------|-------|
| Penny & Low Limit | $1+ (no max) | $1 – $25 | Micro stakes |
| Standard | $5+ (no max) | $5 – $100 | Main floor |
| High Limit | $25+ (no max) | $25 – $500 | High-limit room |
| **401K Contribution** | **$542+ (no max)** | **$542 – $6,500** | Avg. U.S. employee deferral |
| **High Roller / No Limit** | **$2,500 – bankroll** | **bankroll** | Salon stakes — no table maximum |

Cashier: buy up to **$1,000,000** chips per purchase. Cash out to the **private offshore account** up to **$1,000,000,000** per transfer. Offshore withdraws scale with MGM Rewards (Sapphire $1M → Chairman $1B).

## Table Games

### Blackjack

Classic Vegas 21 — solo or full table with AI players. 6-deck shoe, H17, 3:2 blackjack, double/split/surrender/insurance.

→ [Blackjack](blackjack.md)

### Texas Hold'em

No-limit 5-player table (you + 4 bots). Full street flow: preflop → flop → turn → river. Custom raise amounts and all-in supported. Buy-in persists across hands.

→ [Table Games](table-games.md)

### Mandalay Roulette

American wheel (0 and 00). Inside/outside bets with standard payouts.

→ [Table Games](table-games.md)

### Craps

Pass Line / Don't Pass with come-out and point phases. Side bets: Field, Any Craps, Hard Ways, Yo-11.

→ [Table Games](table-games.md)

## Slot Machines

Fourteen machines from penny progressives to high-limit classics. Progressive jackpots (Megabucks, Monte Carlo, Super Spin) persist in your save. **Max bet required** to qualify.

→ [Slot Machines](slots.md)

## Lottery Counter

| Ticket | Description |
|--------|-------------|
| **Strip Pick 3** | 3-digit draw — straight or box |
| **Neon Pick 4** | 4-digit draw — straight or box |
| **Mandalay Mega** | 5 numbers + Mega Ball |
| **Scratchers** | Instant-win tickets |

→ [Lottery Counter](lottery.md)

## Sports Book

Stored scenario board (≥125 events) across NFL, NBA, MLB, MLS, NHL, NCAA, UFC, tennis, and golf. Bet types: moneyline, spread, totals, props, outrights, futures, and 2–4 leg parlays. Binary YES/NO prediction markets (≥125 scenarios) in Polymarket style.

→ [Sports Book](sportsbook.md)

## Trading Floor

Mandalay Markets — long-only futures and call/put options on NYSE equities, commodities, and crypto from a stored contract catalog.

→ [Trading Floor](trading-floor.md)

## Arcade Alley

Vegas-styled classic arcade cabinets in a fullscreen CRT overlay — skill play for chips and redeemable tickets, without entering the pixel RPG.

→ [Arcade Alley](arcade.md)

## Racing Pavilion

Thoroughbred sim with win/place/show wagering. Real horse names, strength-based odds, animated race results.

→ [Racing and Equestrian](racing.md)

## Equestrian Arena

| Event | Description |
|-------|-------------|
| **Dressage** | Scored routine competition |
| **Show Jumping** | Fault-counted round |

→ [Racing and Equestrian](racing.md)

## Beyond the pits

| Amenity | Description |
|---------|-------------|
| **Cashier** | Buy chips, cash out, view ledger |
| **Private Offshore Account** | Park winnings; life/business expense tree (legal, debt, contracts); resort privileges (floor floats, comps, VIP markers, recovery spa) |
| **Casino Floor — shopping & bars** | Mandalay Place sky bridge, three bars, intoxication |
| **Resort Dining** | Aureole, Border Grill, Stripsteak — capacity minigame + drink-scaled encounters |
| **Staff Manifest** | Editable dealer roster with session overrides |
| **High Limit salon** | Chip-gated VIP venue (web + RPG) — private shoe tables, salon-only slots (Obsidian / Whale / Chairman), and a whale sports desk with exclusive tickets |
| **Foundation Room** | Noir+ lounge (web + RPG) — host whisper, Foundation edible; open via host rapport, suite stay, phone call, or bar atmosphere |
| **Gentleman's Club** | Velvet Ledger (web + RPG + CLI) — make it rain, encounters, insanely stocked bar, Tip Cascade / Bottle Memory / Felt Flip, ledger easter eggs; Gold+, suite key, or club phone line |

→ [Resort Dining](dining.md)

## Resort (off the floor)

| Zone | Highlights |
|------|------------|
| **Hotel** | Front desk, hallway mini-game, 17 room vignettes, dining overlay, suite balcony Strip POV smoke-break, Velvet Ledger |
| **Strip Ride** | Web-only limo / Uber / Lyft to Luxor, Excalibur, Bellagio, Circa — exclusive slots + per-property menu themes |
| **Pool complex** | Wave pool, cabanas, Shark Reef, beach club |
| **MGM Rewards** | Tier progression, comps, phone hotline (Connect rideshare unlock) |

→ [Resort Hotel](hotel.md) · [Strip Ride](strip-ride.md) · [Pool Complex](pool-complex.md) · [MGM Rewards](mgm-rewards.md)

## Pixel RPG access

Every major offering above is playable as an in-world encounter in the pixel RPG — no terminal redirect required. **Strip Ride destination floors** are web-terminal only; the RPG has its own walkable Strip overworld.

→ [Pixel RPG](pixel-rpg.md)
